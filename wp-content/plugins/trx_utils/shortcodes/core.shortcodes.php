<?php
/**
 * Logistic Company Framework: shortcodes manipulations
 *
...
// Theme init
if (!function_exists('logistic_company_sc_theme_setup')) {
	add_action( 'logistic_company_action_init_theme', 'logistic_company_sc_theme_setup', 1 );
	function logistic_company_sc_theme_setup() {
		// Add sc stylesheets
		add_action('logistic_company_action_add_styles', 'logistic_company_sc_add_styles', 1);
	}
}

if (!function_exists('logistic_company_sc_theme_setup2')) {
	add_action( 'logistic_company_action_before_init_theme', 'logistic_company_sc_theme_setup2' );
	function logistic_company_sc_theme_setup2() {

		if ( !is_admin() || isset($_POST['action']) ) {
			// Enable/disable shortcodes in excerpt
			add_filter('the_excerpt', 					'logistic_company_sc_excerpt_shortcodes');
	
			// Prepare shortcodes in the content
			if (function_exists('logistic_company_sc_prepare_content')) logistic_company_sc_prepare_content();
		}

		// Add init script into shortcodes output in VC frontend editor
		add_filter('logistic_company_shortcode_output', 'logistic_company_sc_add_scripts', 10, 4);

		// AJAX: Send contact form data
		add_action('wp_ajax_send_form',			'logistic_company_sc_form_send');
		add_action('wp_ajax_nopriv_send_form',	'logistic_company_sc_form_send');

		// Show shortcodes list in admin editor
		add_action('media_buttons',				'logistic_company_sc_selector_add_in_toolbar', 11);
...
if ( !function_exists( 'logistic_company_sc_add_styles' ) ) {
	//add_action('logistic_company_action_add_styles', 'logistic_company_sc_add_styles', 1);
	function logistic_company_sc_add_styles() {
		// Shortcodes
		wp_enqueue_style( 'logistic-company-shortcodes-style',	trx_utils_get_file_url('shortcodes/theme.shortcodes.css'), array(), null );
	}
}


// Register shortcodes init scripts
if ( !function_exists( 'logistic_company_sc_add_scripts' ) ) {
	//add_filter('logistic_company_shortcode_output', 'logistic_company_sc_add_scripts', 10, 4);
	function logistic_company_sc_add_scripts($output, $tag='', $atts=array(), $content='') {

		if (logistic_company_storage_empty('shortcodes_scripts_added')) {
			logistic_company_storage_set('shortcodes_scripts_added', true);
			wp_enqueue_script( 'logistic-company-shortcodes-script', trx_utils_get_file_url('shortcodes/theme.shortcodes.js'), array('jquery'), null, true );
		}
...
if (!function_exists('logistic_company_sc_prepare_content')) {
	function logistic_company_sc_prepare_content() {
		if (function_exists('logistic_company_sc_clear_around')) {
			$filters = array(
				array('logistic_company', 'sc', 'clear', 'around'),
				array('widget', 'text'),
				array('the', 'excerpt'),
				array('the', 'content')
			);
			...
			if (is_array($filters) && count($filters) > 0) {
				foreach ($filters as $flt)
					add_filter(join('_', $flt), 'logistic_company_sc_clear_around', 1);	// Priority 1 to clear spaces before do_shortcodes()
			}
		}
	}
}
...
if (!function_exists('logistic_company_sc_excerpt_shortcodes')) {
	//add_filter('the_excerpt', 'logistic_company_sc_excerpt_shortcodes');
	function logistic_company_sc_excerpt_shortcodes($content) {
		if (!empty($content)) {
...
*/
if (!function_exists('logistic_company_sc_clear_around')) {
	function logistic_company_sc_clear_around($content) {
...
// Logistic Company shortcodes load scripts
if (!function_exists('logistic_company_sc_load_scripts')) {
	function logistic_company_sc_load_scripts() {
		static $loaded = false;
		if (!$loaded) {
			wp_enqueue_script( 'logistic-company-shortcodes_admin-script', trx_utils_get_file_url('shortcodes/shortcodes_admin.js'), array('jquery'), null, true );
			wp_enqueue_script( 'logistic-company-selection-script',  logistic_company_get_file_url('js/jquery.selection.js'), array('jquery'), null, true );
			wp_localize_script( 'logistic-company-shortcodes_admin-script', 'LOGISTIC_COMPANY_SHORTCODES_DATA', logistic_company_storage_get('shortcodes') );
			$loaded = true;
		}
	}
}

// Logistic Company shortcodes prepare scripts
if (!function_exists('logistic_company_sc_prepare_scripts')) {
	function logistic_company_sc_prepare_scripts() {
		static $prepared = false;
		if (!$prepared) {
			logistic_company_storage_set_array('js_vars', 'shortcodes_cp', is_admin() ? (!logistic_company_storage_empty('to_colorpicker') ? logistic_company_storage_get('to_colorpicker') : 'wp') : 'custom');	// wp | tiny | custom
			$prepared = true;
		}
	}
}
...
// Show shortcodes list in admin editor
if (!function_exists('logistic_company_sc_selector_add_in_toolbar')) {
	//add_action('media_buttons','logistic_company_sc_selector_add_in_toolbar', 11);
	function logistic_company_sc_selector_add_in_toolbar(){

		if ( !logistic_company_options_is_used() ) return;

		logistic_company_sc_load_scripts();
		logistic_company_sc_prepare_scripts();

		$shortcodes = logistic_company_storage_get('shortcodes');
		$shortcodes_list = '<select class="sc_selector"><option value="">&nbsp;'.esc_html__('- Select Shortcode -', 'logistic-company').'&nbsp;</option>';
		...
		logistic_company_show_layout($shortcodes_list);
	}
}

// Logistic Company shortcodes builder settings
...
// Logistic Company shortcodes implementation
