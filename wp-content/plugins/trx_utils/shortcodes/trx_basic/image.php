...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('logistic_company_sc_image_theme_setup')) {
	add_action( 'logistic_company_action_before_init_theme', 'logistic_company_sc_image_theme_setup' );
	function logistic_company_sc_image_theme_setup() {
		add_action('logistic_company_action_shortcodes_list', 		'logistic_company_sc_image_reg_shortcodes');
		if (function_exists('logistic_company_exists_visual_composer') && logistic_company_exists_visual_composer())
			add_action('logistic_company_action_shortcodes_list_vc','logistic_company_sc_image_reg_shortcodes_vc');
	}
}
...
if (!function_exists('logistic_company_sc_image')) {	
	function logistic_company_sc_image($atts, $content=null){	
		...
		if (!empty($width) || !empty($height)) {
			...
			if ($w || $h) $src = logistic_company_get_resized_image_url($src, $w, $h);
		}
		if (trim($link)) logistic_company_enqueue_popup();
		$output = empty($src) ? '' : ('<figure' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			...
			. '</figure>');
		return apply_filters('logistic_company_shortcode_output', $output, 'trx_image', $atts, $content);
	}
	logistic_company_require_shortcode('trx_image', 'logistic_company_sc_image');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'logistic_company_sc_image_reg_shortcodes' ) ) {
	//add_action('logistic_company_action_shortcodes_list', 'logistic_company_sc_image_reg_shortcodes');
	function logistic_company_sc_image_reg_shortcodes() {
	
		logistic_company_sc_map("trx_image", array(
			"title" => esc_html__("Image", 'logistic-company'),
			"desc" => wp_kses_data( __("Insert image into your post (page)", 'logistic-company') ),
			"decorate" => false,
			"container" => false,
			"params" => array(
				"url" => array(
					"title" => esc_html__("URL for image file", 'logistic-company'),
					"desc" => wp_kses_data( __("Select or upload image or write URL from other site", 'logistic-company') ),
					"readonly" => false,
					"value" => "",
					"type" => "media",
					"before" => array(
				...
				"title" => array(
					...
					"desc" => wp_kses_data( __("Image title (if need)", 'logistic-company') ),
					"value" => "",
					"type" => "text"
				),
				"icon" => array(
					"title" => esc_html__("Icon before title",  'logistic-company'),
				...
				"align" => array(
					"title" => esc_html__("Float image", 'logistic-company'),
					"desc" => wp_kses_data( __("Float image to left or right side", 'logistic-company') ),
					"value" => "",
					"type" => "checklist",
					"dir" => "horizontal",
				...
				"shape" => array(
					"title" => esc_html__("Image Shape", 'logistic-company'),
					"desc" => wp_kses_data( __("Shape of the image: square (rectangle) or round", 'logistic-company') ),
					"value" => "square",
					"type" => "checklist",
					"dir" => "horizontal",
					"options" => array(
				...
				"link" => array(
					...
					"desc" => wp_kses_data( __("The link URL from the image", 'logistic-company') ),
					"value" => "",
					"type" => "text"
				),
...
-------------------------------------------------------------------- */
if ( !function_exists( 'logistic_company_sc_image_reg_shortcodes_vc' ) ) {
	//add_action('logistic_company_action_shortcodes_list_vc', 'logistic_company_sc_image_reg_shortcodes_vc');
	function logistic_company_sc_image_reg_shortcodes_vc() {
	
		vc_map( array(
			"base" => "trx_image",
			"name" => esc_html__("Image", 'logistic-company'),
			"description" => wp_kses_data( __("Insert image", 'logistic-company') ),
			...
			"params" => array(
				array(
					"param_name" => "url",
					"heading" => esc_html__("Select image", 'logistic-company'),
					"description" => wp_kses_data( __("Select image from library", 'logistic-company') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "attach_image"
				...
				array(
					...
					"description" => wp_kses_data( __("Align image to left or right side", 'logistic-company') ),
					"admin_label" => true,
					"class" => "",
				...
				array(
					"param_name" => "shape",
					"heading" => esc_html__("Image shape", 'logistic-company'),
					"description" => wp_kses_data( __("Shape of the image: square or round", 'logistic-company') ),
					"admin_label" => true,
					"class" => "",
					"value" => array(
				...
				array(
					...
					"description" => wp_kses_data( __("Image's title", 'logistic-company') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
				...
				array(
					"param_name" => "icon",
					"heading" => esc_html__("Title's icon", 'logistic-company'),
				...
				array(
					...
					"description" => wp_kses_data( __("The link URL from the image", 'logistic-company') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
		...
		class WPBakeryShortCode_Trx_Image extends LOGISTIC_COMPANY_VC_ShortCodeSingle {}
	}
}
?>