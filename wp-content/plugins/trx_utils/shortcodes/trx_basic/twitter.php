...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('logistic_company_sc_twitter_theme_setup')) {
	add_action( 'logistic_company_action_before_init_theme', 'logistic_company_sc_twitter_theme_setup' );
	function logistic_company_sc_twitter_theme_setup() {
		add_action('logistic_company_action_shortcodes_list', 		'logistic_company_sc_twitter_reg_shortcodes');
		if (function_exists('logistic_company_exists_visual_composer') && logistic_company_exists_visual_composer())
			add_action('logistic_company_action_shortcodes_list_vc','logistic_company_sc_twitter_reg_shortcodes_vc');
	}
}
...
if (!function_exists('logistic_company_sc_twitter')) {	
	function logistic_company_sc_twitter($atts, $content=null){	
		...
		$twitter_username = $user ? $user : logistic_company_get_theme_option('twitter_username');
		$twitter_consumer_key = $consumer_key ? $consumer_key : logistic_company_get_theme_option('twitter_consumer_key');
		$twitter_consumer_secret = $consumer_secret ? $consumer_secret : logistic_company_get_theme_option('twitter_consumer_secret');
		$twitter_token_key = $token_key ? $token_key : logistic_company_get_theme_option('twitter_token_key');
		$twitter_token_secret = $token_secret ? $token_secret : logistic_company_get_theme_option('twitter_token_secret');
		$twitter_count = max(1, $count ? $count : intval(logistic_company_get_theme_option('twitter_count')));
		...
		if (!empty($twitter_consumer_key) && !empty($twitter_consumer_secret) && !empty($twitter_token_key) && !empty($twitter_token_secret)) {
			$data = logistic_company_get_twitter_data(array(
				'mode'            => 'user_timeline',
			...
			if ($data && isset($data[0]['text'])) {
				logistic_company_enqueue_slider('swiper');
				$output = ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || logistic_company_strlen($bg_texture)>2 || ($scheme && !logistic_company_param_is_off($scheme) && !logistic_company_param_is_inherit($scheme))
						? '<div class="sc_twitter_wrap sc_section'
								...
								. ($align && $align!='none' && !logistic_company_param_is_inherit($align) ? ' align' . esc_attr($align) : '')
								. '"'
							.' style="'
						...
						. '<div class="sc_twitter'
								...
								. ($bg_color=='' && $bg_image=='' && $bg_overlay==0 && ($bg_texture=='' || $bg_texture=='0') && $align && $align!='none' && !logistic_company_param_is_inherit($align) ? ' align' . esc_attr($align) : '')
								. '"'
				...
				if (is_array($data) && count($data) > 0) {
					foreach ($data as $tweet) {
						if (logistic_company_substr($tweet['text'], 0, 1)=='@') continue;
							$output .= '<div class="swiper-slide" data-style="'.esc_attr(($ws).($hs)).'" style="'.esc_attr(($ws).($hs)).'">'
										. '<div class="sc_twitter_item">'
											...
											. '<div class="sc_twitter_content">'
												...
												. wp_kses_data(logistic_company_prepare_twitter_text($tweet))
											. '</div>'
										. '</div>'
									. '</div>';
				...
				$output .= '</div>'
						...
						. '</div>'
					. '</div>'
					. ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || logistic_company_strlen($bg_texture)>2
						?  '</div></div>'
						: '');
			}
		}
		return apply_filters('logistic_company_shortcode_output', $output, 'trx_twitter', $atts, $content);
	}
	logistic_company_require_shortcode('trx_twitter', 'logistic_company_sc_twitter');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'logistic_company_sc_twitter_reg_shortcodes' ) ) {
	//add_action('logistic_company_action_shortcodes_list', 'logistic_company_sc_twitter_reg_shortcodes');
	function logistic_company_sc_twitter_reg_shortcodes() {
	
		logistic_company_sc_map("trx_twitter", array(
			"title" => esc_html__("Twitter", 'logistic-company'),
			"desc" => wp_kses_data( __("Insert twitter feed into post (page)", 'logistic-company') ),
			"decorate" => false,
			"container" => false,
			"params" => array(
				"user" => array(
					"title" => esc_html__("Twitter Username", 'logistic-company'),
					"desc" => wp_kses_data( __("Your username in the twitter account. If empty - get it from Theme Options.", 'logistic-company') ),
					"value" => "",
					"type" => "text"
				),
				"consumer_key" => array(
					"title" => esc_html__("Consumer Key", 'logistic-company'),
					"desc" => wp_kses_data( __("Consumer Key from the twitter account", 'logistic-company') ),
					"value" => "",
					"type" => "text"
				),
				"consumer_secret" => array(
					"title" => esc_html__("Consumer Secret", 'logistic-company'),
					"desc" => wp_kses_data( __("Consumer Secret from the twitter account", 'logistic-company') ),
					"value" => "",
					"type" => "text"
				),
				"token_key" => array(
					"title" => esc_html__("Token Key", 'logistic-company'),
					"desc" => wp_kses_data( __("Token Key from the twitter account", 'logistic-company') ),
					"value" => "",
					"type" => "text"
				),
				"token_secret" => array(
					"title" => esc_html__("Token Secret", 'logistic-company'),
					"desc" => wp_kses_data( __("Token Secret from the twitter account", 'logistic-company') ),
					"value" => "",
					"type" => "text"
				),
				"count" => array(
					"title" => esc_html__("Tweets number", 'logistic-company'),
					"desc" => wp_kses_data( __("Tweets number to show", 'logistic-company') ),
					"divider" => true,
					"value" => 3,
					"max" => 20,
					"min" => 1,
				...
				"controls" => array(
					"title" => esc_html__("Show arrows", 'logistic-company'),
					"desc" => wp_kses_data( __("Show control buttons", 'logistic-company') ),
					"value" => "yes",
					"type" => "switch",
					"options" => logistic_company_get_sc_param('yes_no')
				),
				"interval" => array(
					"title" => esc_html__("Tweets change interval", 'logistic-company'),
					"desc" => wp_kses_data( __("Tweets change interval (in milliseconds: 1000ms = 1s)", 'logistic-company') ),
					"value" => 7000,
					"step" => 500,
					"min" => 0,
					"type" => "spinner"
				...
				"align" => array(
					...
					"desc" => wp_kses_data( __("Alignment of the tweets block", 'logistic-company') ),
					"divider" => true,
					"value" => "",
					"type" => "checklist",
					"dir" => "horizontal",
...
-------------------------------------------------------------------- */
if ( !function_exists( 'logistic_company_sc_twitter_reg_shortcodes_vc' ) ) {
	//add_action('logistic_company_action_shortcodes_list_vc', 'logistic_company_sc_twitter_reg_shortcodes_vc');
	function logistic_company_sc_twitter_reg_shortcodes_vc() {
	
		vc_map( array(
			"base" => "trx_twitter",
			"name" => esc_html__("Twitter", 'logistic-company'),
			"description" => wp_kses_data( __("Insert twitter feed into post (page)", 'logistic-company') ),
			...
			"params" => array(
				array(
					"param_name" => "user",
					"heading" => esc_html__("Twitter Username", 'logistic-company'),
					"description" => wp_kses_data( __("Your username in the twitter account. If empty - get it from Theme Options.", 'logistic-company') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					...
					"heading" => esc_html__("Consumer Key", 'logistic-company'),
					"description" => wp_kses_data( __("Consumer Key from the twitter account", 'logistic-company') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					...
					"heading" => esc_html__("Consumer Secret", 'logistic-company'),
					"description" => wp_kses_data( __("Consumer Secret from the twitter account", 'logistic-company') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					...
					"heading" => esc_html__("Token Key", 'logistic-company'),
					"description" => wp_kses_data( __("Token Key from the twitter account", 'logistic-company') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					...
					"heading" => esc_html__("Token Secret", 'logistic-company'),
					"description" => wp_kses_data( __("Token Secret from the twitter account", 'logistic-company') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					"param_name" => "count",
					"heading" => esc_html__("Tweets number", 'logistic-company'),
					"description" => wp_kses_data( __("Number tweets to show", 'logistic-company') ),
					"class" => "",
					"divider" => true,
					"value" => 3,
					"type" => "textfield"
				...
				array(
					"param_name" => "controls",
					"heading" => esc_html__("Show arrows", 'logistic-company'),
					"description" => wp_kses_data( __("Show control buttons", 'logistic-company') ),
					"admin_label" => true,
					"class" => "",
				...
				array(
					"param_name" => "interval",
					"heading" => esc_html__("Tweets change interval", 'logistic-company'),
					"description" => wp_kses_data( __("Tweets change interval (in milliseconds: 1000ms = 1s)", 'logistic-company') ),
					"class" => "",
					"value" => "7000",
					"type" => "textfield"
				),
				array(
					...
					"description" => wp_kses_data( __("Alignment of the tweets block", 'logistic-company') ),
					"class" => "",
		...
		class WPBakeryShortCode_Trx_Twitter extends LOGISTIC_COMPANY_VC_ShortCodeSingle {}
	}
}
?>