...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('logistic_company_sc_parallax_theme_setup')) {
	add_action( 'logistic_company_action_before_init_theme', 'logistic_company_sc_parallax_theme_setup' );
	function logistic_company_sc_parallax_theme_setup() {
		add_action('logistic_company_action_shortcodes_list', 		'logistic_company_sc_parallax_reg_shortcodes');
		if (function_exists('logistic_company_exists_visual_composer') && logistic_company_exists_visual_composer())
			add_action('logistic_company_action_shortcodes_list_vc','logistic_company_sc_parallax_reg_shortcodes_vc');
	}
}
...
if (!function_exists('logistic_company_sc_parallax')) {	
	function logistic_company_sc_parallax($atts, $content=null){	
		...
		if ($bg_video!='') {
			...
			$bg_video_width = !empty($width) && logistic_company_substr($width, -1) >= '0' && logistic_company_substr($width, -1) <= '9'  ? $width : 1280;
		...
		$css .= logistic_company_get_css_dimensions_from_values($width, $height)
			...
			;
		$output = (logistic_company_param_is_on($gap) ? logistic_company_gap_start() : '')
			...
			. '</div>'
			. '</div>'
			. (logistic_company_param_is_on($gap) ? logistic_company_gap_end() : '');
		return apply_filters('logistic_company_shortcode_output', $output, 'trx_parallax', $atts, $content);
	}
	logistic_company_require_shortcode('trx_parallax', 'logistic_company_sc_parallax');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'logistic_company_sc_parallax_reg_shortcodes' ) ) {
	//add_action('logistic_company_action_shortcodes_list', 'logistic_company_sc_parallax_reg_shortcodes');
	function logistic_company_sc_parallax_reg_shortcodes() {
	
		logistic_company_sc_map("trx_parallax", array(
			"title" => esc_html__("Parallax", 'logistic-company'),
			"desc" => wp_kses_data( __("Create the parallax container (with asinc background image)", 'logistic-company') ),
			"decorate" => false,
			"container" => true,
			"params" => array(
				"gap" => array(
					"title" => esc_html__("Create gap", 'logistic-company'),
					"desc" => wp_kses_data( __("Create gap around parallax container", 'logistic-company') ),
					"value" => "no",
					"size" => "small",
				...
				"dir" => array(
					"title" => esc_html__("Dir", 'logistic-company'),
					"desc" => wp_kses_data( __("Scroll direction for the parallax background", 'logistic-company') ),
					"value" => "up",
					"size" => "medium",
					"options" => array(
						'up' => esc_html__('Up', 'logistic-company'),
						'down' => esc_html__('Down', 'logistic-company')
					),
					"type" => "switch"
				), 
				"speed" => array(
					"title" => esc_html__("Speed", 'logistic-company'),
					"desc" => wp_kses_data( __("Image motion speed (from 0.0 to 1.0)", 'logistic-company') ),
					"min" => "0",
					"max" => "1",
					"step" => "0.1",
					"value" => "0.3",
				...
				"color" => array(
					...
					"desc" => wp_kses_data( __("Select color for text object inside parallax block", 'logistic-company') ),
					"divider" => true,
					"value" => "",
					"type" => "color"
				),
				"bg_color" => array(
					...
					"desc" => wp_kses_data( __("Select color for parallax background", 'logistic-company') ),
					"value" => "",
					"type" => "color"
				),
				"bg_image" => array(
					"title" => esc_html__("Background image", 'logistic-company'),
					"desc" => wp_kses_data( __("Select or upload image or write URL from other site for the parallax background", 'logistic-company') ),
					"readonly" => false,
					"value" => "",
					"type" => "media"
				),
				"bg_image_x" => array(
					"title" => esc_html__("Image X position", 'logistic-company'),
					"desc" => wp_kses_data( __("Image horizontal position (as background of the parallax block) - in percent", 'logistic-company') ),
					"min" => "0",
					"max" => "100",
					"value" => "50",
					"type" => "spinner"
				...
				"bg_video" => array(
					...
					"desc" => wp_kses_data( __("Select video from media library or paste URL for video file from other site to show it as parallax background", 'logistic-company') ),
					"readonly" => false,
					"value" => "",
					"type" => "media",
					"before" => array(
				...
				"bg_video_ratio" => array(
					"title" => esc_html__("Video ratio", 'logistic-company'),
					"desc" => wp_kses_data( __("Specify ratio of the video background. For example: 16:9 (default), 4:3, etc.", 'logistic-company') ),
					"value" => "16:9",
					"type" => "text"
				),
				"bg_overlay" => array(
				...
				"_content_" => array(
					"title" => esc_html__("Content", 'logistic-company'),
					"desc" => wp_kses_data( __("Content for the parallax container", 'logistic-company') ),
					"divider" => true,
					"value" => "",
					"type" => "text"
				),
...
-------------------------------------------------------------------- */
if ( !function_exists( 'logistic_company_sc_parallax_reg_shortcodes_vc' ) ) {
	//add_action('logistic_company_action_shortcodes_list_vc', 'logistic_company_sc_parallax_reg_shortcodes_vc');
	function logistic_company_sc_parallax_reg_shortcodes_vc() {
	
		vc_map( array(
			"base" => "trx_parallax",
			"name" => esc_html__("Parallax", 'logistic-company'),
			"description" => wp_kses_data( __("Create the parallax container (with asinc background image)", 'logistic-company') ),
			"category" => esc_html__('Structure', 'logistic-company'),
			...
			"params" => array(
				array(
					"param_name" => "gap",
					"heading" => esc_html__("Create gap", 'logistic-company'),
					"description" => wp_kses_data( __("Create gap around parallax container (not need in fullscreen pages)", 'logistic-company') ),
					"class" => "",
					"value" => array(esc_html__('Create gap', 'logistic-company') => 'yes'),
					"type" => "checkbox"
				),
				array(
					"param_name" => "dir",
					"heading" => esc_html__("Direction", 'logistic-company'),
					"description" => wp_kses_data( __("Scroll direction for the parallax background", 'logistic-company') ),
					"admin_label" => true,
					"class" => "",
					"value" => array(
							esc_html__('Up', 'logistic-company') => 'up',
							esc_html__('Down', 'logistic-company') => 'down'
						),
					"type" => "dropdown"
				),
				array(
					"param_name" => "speed",
					"heading" => esc_html__("Speed", 'logistic-company'),
					"description" => wp_kses_data( __("Parallax background motion speed (from 0.0 to 1.0)", 'logistic-company') ),
					"class" => "",
					"value" => "0.3",
					"type" => "textfield"
				),
				...
				array(
					...
					"description" => wp_kses_data( __("Select color for text object inside parallax block", 'logistic-company') ),
					"class" => "",
					"value" => "",
					"type" => "colorpicker"
				),
				array(
					"param_name" => "bg_color",
					"heading" => esc_html__("Backgroud color", 'logistic-company'),
					"description" => wp_kses_data( __("Select color for parallax background", 'logistic-company') ),
					"class" => "",
					"value" => "",
					"type" => "colorpicker"
				),
				array(
					...
					"description" => wp_kses_data( __("Select or upload image or write URL from other site for the parallax background", 'logistic-company') ),
					"class" => "",
					"value" => "",
					"type" => "attach_image"
				),
				array(
					...
					"heading" => esc_html__("Image X position", 'logistic-company'),
					"description" => wp_kses_data( __("Parallax background X position (in percents)", 'logistic-company') ),
					"class" => "",
					"value" => "50%",
					"type" => "textfield"
				),
				array(
					"param_name" => "bg_video",
					"heading" => esc_html__("Video background", 'logistic-company'),
					"description" => wp_kses_data( __("Paste URL for video file to show it as parallax background", 'logistic-company') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					...
					"heading" => esc_html__("Video ratio", 'logistic-company'),
					"description" => wp_kses_data( __("Specify ratio of the video background. For example: 16:9 (default), 4:3, etc.", 'logistic-company') ),
					"class" => "",
					"value" => "16:9",
					"type" => "textfield"
				),
		...
		class WPBakeryShortCode_Trx_Parallax extends LOGISTIC_COMPANY_VC_ShortCodeCollection {}
	}
}
?>