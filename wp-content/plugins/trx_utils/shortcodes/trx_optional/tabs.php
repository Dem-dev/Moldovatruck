...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('logistic_company_sc_tabs_theme_setup')) {
	add_action( 'logistic_company_action_before_init_theme', 'logistic_company_sc_tabs_theme_setup' );
	function logistic_company_sc_tabs_theme_setup() {
		add_action('logistic_company_action_shortcodes_list', 		'logistic_company_sc_tabs_reg_shortcodes');
		if (function_exists('logistic_company_exists_visual_composer') && logistic_company_exists_visual_composer())
			add_action('logistic_company_action_shortcodes_list_vc','logistic_company_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('logistic_company_sc_tabs')) {	
	function logistic_company_sc_tabs($atts, $content = null) {
		...
		logistic_company_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = logistic_company_storage_get_array('sc_tab_data', 'titles');
		...
		$tabs_output .= $titles_output
			...
			. ($content)
			.'</div>';
		return apply_filters('logistic_company_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	logistic_company_require_shortcode("trx_tabs", "logistic_company_sc_tabs");
}


if (!function_exists('logistic_company_sc_tab')) {	
	function logistic_company_sc_tab($atts, $content = null) {
		if (logistic_company_in_shortcode_blogger()) return '';
		extract(logistic_company_html_decode(shortcode_atts(array(
			// Individual params
		...
		logistic_company_storage_inc_array('sc_tab_data', 'counter');
		$counter = logistic_company_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : logistic_company_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = logistic_company_storage_get_array('sc_tab_data', 'titles');
		...
		logistic_company_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (logistic_company_param_is_on(logistic_company_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(logistic_company_storage_get_array('sc_tab_data', 'height') != '' ? logistic_company_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (logistic_company_param_is_on(logistic_company_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('logistic_company_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	logistic_company_require_shortcode("trx_tab", "logistic_company_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'logistic_company_sc_tabs_reg_shortcodes' ) ) {
	//add_action('logistic_company_action_shortcodes_list', 'logistic_company_sc_tabs_reg_shortcodes');
	function logistic_company_sc_tabs_reg_shortcodes() {
	
		logistic_company_sc_map("trx_tabs", array(
			"title" => esc_html__("Tabs", 'logistic-company'),
			"desc" => wp_kses_data( __("Insert tabs in your page (post)", 'logistic-company') ),
			"decorate" => true,
			"container" => false,
			"params" => array(
				"style" => array(
					"title" => esc_html__("Tabs style", 'logistic-company'),
					"desc" => wp_kses_data( __("Select style for tabs items", 'logistic-company') ),
					"value" => 1,
					"options" => logistic_company_get_list_styles(1, 2),
					"type" => "radio"
				),
				"initial" => array(
					"title" => esc_html__("Initially opened tab", 'logistic-company'),
					"desc" => wp_kses_data( __("Number of initially opened tab", 'logistic-company') ),
					"divider" => true,
					"value" => 1,
					"min" => 0,
					"type" => "spinner"
				...
				"scroll" => array(
					...
					"desc" => wp_kses_data( __("Use scroller to show tab content (height parameter required)", 'logistic-company') ),
					"divider" => true,
					"value" => "no",
					"type" => "switch",
			...
			"children" => array(
				"name" => "trx_tab",
				"title" => esc_html__("Tab", 'logistic-company'),
				"desc" => wp_kses_data( __("Tab item", 'logistic-company') ),
				"container" => true,
				"params" => array(
					"title" => array(
						"title" => esc_html__("Tab title", 'logistic-company'),
						"desc" => wp_kses_data( __("Current tab title", 'logistic-company') ),
						"value" => "",
						"type" => "text"
					),
					"_content_" => array(
						"title" => esc_html__("Tab content", 'logistic-company'),
						"desc" => wp_kses_data( __("Current tab content", 'logistic-company') ),
						"divider" => true,
						"rows" => 4,
						"value" => "",
						"type" => "textarea"
...
-------------------------------------------------------------------- */
if ( !function_exists( 'logistic_company_sc_tabs_reg_shortcodes_vc' ) ) {
	//add_action('logistic_company_action_shortcodes_list_vc', 'logistic_company_sc_tabs_reg_shortcodes_vc');
	function logistic_company_sc_tabs_reg_shortcodes_vc() {
	
		$tab_id_1 = 'sc_tab_'.time() . '_1_' . rand( 0, 100 );
		$tab_id_2 = 'sc_tab_'.time() . '_2_' . rand( 0, 100 );
		vc_map( array(
			"base" => "trx_tabs",
			"name" => esc_html__("Tabs", 'logistic-company'),
			"desc" => wp_kses_data( __("Tabs", 'logistic-company') ),
			...
			"params" => array(
				array(
					"param_name" => "style",
					"heading" => esc_html__("Tabs style", 'logistic-company'),
					"desc" => wp_kses_data( __("Select style of tabs items", 'logistic-company') ),
					"admin_label" => true,
					"class" => "",
				...
				array(
					"param_name" => "initial",
					"heading" => esc_html__("Initially opened tab", 'logistic-company'),
					"desc" => wp_kses_data( __("Number of initially opened tab", 'logistic-company') ),
					"class" => "",
					"value" => 1,
					"type" => "textfield"
				...
				array(
					"param_name" => "scroll",
					"heading" => esc_html__("Scroller", 'logistic-company'),
					"desc" => wp_kses_data( __("Use scroller to show tab content (height parameter required)", 'logistic-company') ),
					"class" => "",
			...
			'default_content' => '
				[trx_tab title="' . esc_html__( 'Tab 1', 'logistic-company' ) . '" tab_id="'.esc_attr($tab_id_1).'"][/trx_tab]
				[trx_tab title="' . esc_html__( 'Tab 2', 'logistic-company' ) . '" tab_id="'.esc_attr($tab_id_2).'"][/trx_tab]
			',
			"custom_markup" => '
		...
		vc_map( array(
			"base" => "trx_tab",
			"name" => esc_html__("Tab item", 'logistic-company'),
			"desc" => wp_kses_data( __("Single tab item", 'logistic-company') ),
			...
			"params" => array(
				array(
					"param_name" => "title",
					"heading" => esc_html__("Tab title", 'logistic-company'),
					"desc" => wp_kses_data( __("Title for current tab", 'logistic-company') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
				...
				array(
					"param_name" => "tab_id",
					"heading" => esc_html__("Tab ID", 'logistic-company'),
					"desc" => wp_kses_data( __("ID for current tab (required). Please, start it from letter.", 'logistic-company') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
		...
		class WPBakeryShortCode_Trx_Tabs extends LOGISTIC_COMPANY_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends LOGISTIC_COMPANY_VC_ShortCodeTab {}
	}
}
?>