...
( function( $ ) {
	...
	if (!logistic_company_previewer_vars['need_refresh']) {
		var $style = $('#logistic_company-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="logistic_company-customizer-css"></'+'style'+'>' ).find('#logistic_company-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			logistic_company_set_cookie(logistic_company_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
