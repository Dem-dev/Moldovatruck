// Logistic Company Customizer script
...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.logistic_company_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.logistic_company_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && logistic_company_get_cookie('to_demo_fixed')==1) logistic_company_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.logistic_company_options_field_info .to_demo_pin', function(e) {
		logistic_company_options_fix_demo(jQuery(this).parents('.logistic_company_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	logistic_company_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.logistic_company_options_input,.logistic_company_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.logistic_company_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		logistic_company_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.logistic_company_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.logistic_company_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .logistic_company_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.logistic_company_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? LOGISTIC_COMPANY_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.logistic_company_options_tab').find('> .logistic_company_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="logistic_company_options_schemes_order" class="logistic_company_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function logistic_company_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.logistic_company_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.logistic_company_options').find('>.logistic_company_options_field_info');
...
function logistic_company_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.logistic_company_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	logistic_company_message_confirm(LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_delete_confirm'], LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'logistic_company_options_scheme_delete',
			nonce: LOGISTIC_COMPANY_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.logistic_company_options_tab').find('> .logistic_company_options_schemes_order').val()
		};
		setTimeout(function(){
			logistic_company_message_info(LOGISTIC_COMPANY_STORAGE['to_strings']['recompile_styles'], LOGISTIC_COMPANY_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(LOGISTIC_COMPANY_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				logistic_company_message_success(LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+LOGISTIC_COMPANY_STORAGE['to_strings']['reload_page'], LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				logistic_company_message_warning(LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function logistic_company_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.logistic_company_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	logistic_company_message_confirm(LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_copy_confirm'], LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'logistic_company_options_scheme_copy',
			nonce: LOGISTIC_COMPANY_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.logistic_company_options_tab').find('> .logistic_company_options_schemes_order').val()
		};
		...
		jQuery.post(LOGISTIC_COMPANY_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				logistic_company_message_success(LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+LOGISTIC_COMPANY_STORAGE['to_strings']['reload_page'], LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				logistic_company_message_warning(LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, LOGISTIC_COMPANY_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function logistic_company_options_fix_demo(info) {
	var info2 = info.parents('.logistic_company_options').toggleClass('to_demo_fixed').find('>.logistic_company_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.logistic_company_options').append(clonedObj);
		info2 = info.parents('.logistic_company_options').find('>.logistic_company_options_field_info');
	}
	logistic_company_set_cookie('to_demo_fixed', jQuery('.logistic_company_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function logistic_company_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.logistic_company_options_field_info').data('scheme');
