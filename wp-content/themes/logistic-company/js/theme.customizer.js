...
function logistic_company_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = LOGISTIC_COMPANY_STORAGE['co_add_params']!=undefined ? LOGISTIC_COMPANY_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#logistic_company-customizer-styles-'+option).length > 0 ? jQuery('#logistic_company-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="logistic_company-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
