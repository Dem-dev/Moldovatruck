<?php
/**
 * Logistic Company Framework: Testimonial support
 *
...
// Theme init
if (!function_exists('logistic_company_testimonial_theme_setup')) {
	add_action( 'logistic_company_action_before_init_theme', 'logistic_company_testimonial_theme_setup', 1 );
	function logistic_company_testimonial_theme_setup() {
	
		// Add item in the admin menu
		add_filter('trx_utils_filter_override_options',		'logistic_company_testimonial_add_override_options');

		// Save data from override options
		add_action('save_post',				'logistic_company_testimonial_save_data');

		// Register shortcodes [trx_testimonials] and [trx_testimonials_item]
		add_action('logistic_company_action_shortcodes_list',		'logistic_company_testimonials_reg_shortcodes');
		if (function_exists('logistic_company_exists_visual_composer') && logistic_company_exists_visual_composer())
			add_action('logistic_company_action_shortcodes_list_vc','logistic_company_testimonials_reg_shortcodes_vc');

		// Meta box fields
		logistic_company_storage_set('testimonial_override_options', array(
			'id' => 'testimonial-override-options',
			'title' => esc_html__('Testimonial Details', 'logistic-company'),
			'page' => 'testimonial',
			'context' => 'normal',
			'priority' => 'high',
			'fields' => array(
				"testimonial_author" => array(
					"title" => esc_html__('Testimonial author',  'logistic-company'),
					"desc" => wp_kses_data( __("Name of the testimonial's author", 'logistic-company') ),
					"class" => "testimonial_author",
					"std" => "",
					"type" => "text"),
				"testimonial_position" => array(
					"title" => esc_html__("Author's position",  'logistic-company'),
					"desc" => wp_kses_data( __("Position of the testimonial's author", 'logistic-company') ),
					"class" => "testimonial_author",
					"std" => "",
					"type" => "text"),
				"testimonial_email" => array(
					"title" => esc_html__("Author's e-mail",  'logistic-company'),
					"desc" => wp_kses_data( __("E-mail of the testimonial's author - need to take Gravatar (if registered)", 'logistic-company') ),
					"class" => "testimonial_email",
					"std" => "",
					"type" => "text"),
				"testimonial_link" => array(
					"title" => esc_html__('Testimonial link',  'logistic-company'),
					"desc" => wp_kses_data( __("URL of the testimonial source or author profile page", 'logistic-company') ),
					"class" => "testimonial_link",
					"std" => "",
					"type" => "text"),
                "testimonial_title" => array(
                    "title" => esc_html__('Testimonial title',  'logistic-company'),
                    "desc" => wp_kses_data( __("Testimonial title", 'logistic-company') ),
		...
		logistic_company_theme_support_pt('testimonial');
		logistic_company_theme_support_tx('testimonial_group');
...
if (!function_exists('logistic_company_testimonial_add_override_options')) {
    //add_action('trx_utils_filter_override_options', 'logistic_company_testimonial_add_override_options');
    function logistic_company_testimonial_add_override_options($boxes = array()) {
        $boxes[] = array_merge(logistic_company_storage_get('testimonial_override_options'), array('callback' => 'logistic_company_testimonial_show_override_options'));
        return $boxes;
    }
}
...
if (!function_exists('logistic_company_testimonial_show_override_options')) {
	function logistic_company_testimonial_show_override_options() {
		global $post;

		// Use nonce for verification
		echo '<input type="hidden" name="override_options_testimonial_nonce" value="'.esc_attr(wp_create_nonce(admin_url())).'" />';
		
		$data = get_post_meta($post->ID, logistic_company_storage_get('options_prefix').'_testimonial_data', true);
	
		$fields = logistic_company_storage_get_array('testimonial_override_options', 'fields');
		?>
...
// Save data from override options
if (!function_exists('logistic_company_testimonial_save_data')) {
	//Handler of add_action('save_post', 'logistic_company_testimonial_save_data');
	function logistic_company_testimonial_save_data($post_id) {
		// verify nonce
		if ( !wp_verify_nonce( logistic_company_get_value_gp('override_options_testimonial_nonce'), admin_url() ) )
			return $post_id;
		...
		update_post_meta($post_id, logistic_company_storage_get('options_prefix').'_testimonial_data', $data);
	}
}
...
if (!function_exists('logistic_company_sc_testimonials')) {
	function logistic_company_sc_testimonials($atts, $content=null){	
		...
		logistic_company_storage_set('sc_testimonials_data', array(
			'id' => $id,
            'style' => $style,
            'columns' => $columns,
            'counter' => 0,
		...
		$output = ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || logistic_company_strlen($bg_texture)>2 || ($scheme && !logistic_company_param_is_off($scheme) && !logistic_company_param_is_inherit($scheme))
				...
				. ' class="sc_testimonials sc_testimonials_style_'.esc_attr($style)
					...
					. '"'
				. ($bg_color=='' && $bg_image=='' && $bg_overlay==0 && ($bg_texture=='' || $bg_texture=='0') && !logistic_company_param_is_off($animation) ? ' data-animation="'.esc_attr(logistic_company_get_animation_classes($animation)).'"' : '')
				. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. '>'
			. (!empty($subtitle) ? '<h6 class="sc_testimonials_subtitle sc_item_subtitle">' . trim(logistic_company_strmacros($subtitle)) . '</h6>' : '')
			. (!empty($title) ? '<h2 class="sc_testimonials_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(logistic_company_strmacros($title)) . '</h2>' : '')
			. (!empty($description) ? '<div class="sc_testimonials_descr sc_item_descr">' . trim(logistic_company_strmacros($description)) . '</div>' : '')
		...
		if (logistic_company_param_is_on($custom) && $content) {
			...
			$args = logistic_company_query_add_posts_and_cats($args, $ids, 'testimonial', $cat, 'testimonial_group');
			...
			while ( $query->have_posts() ) { 
				...
				$post_meta = get_post_meta($post_data['post_id'], logistic_company_storage_get('options_prefix').'_testimonial_data', true);
                ...
                $args['t_title'] = $post_meta['testimonial_title'];
		...
		$output .= '</div>'
					. ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || logistic_company_strlen($bg_texture)>2 || ($scheme && !logistic_company_param_is_off($scheme) && !logistic_company_param_is_inherit($scheme))
						?  '</div></div>'
						: '');
		...
		return apply_filters('logistic_company_shortcode_output', $output, 'trx_testimonials', $atts, $content);
	}
	logistic_company_require_shortcode('trx_testimonials', 'logistic_company_sc_testimonials');
}
	
	
if (!function_exists('logistic_company_sc_testimonials_item')) {
	function logistic_company_sc_testimonials_item($atts, $content=null){	
		if (logistic_company_in_shortcode_blogger()) return '';
		extract(logistic_company_html_decode(shortcode_atts(array(
			// Individual params
			"author" => "",
		...
		logistic_company_storage_inc_array('sc_testimonials_data', 'counter');
	
		$id = $id ? $id : (logistic_company_storage_get_array('sc_testimonials_data', 'id') ? logistic_company_storage_get_array('sc_testimonials_data', 'id') . '_' . logistic_company_storage_get_array('sc_testimonials_data', 'counter') : '');
	
		$thumb_sizes = logistic_company_get_thumb_sizes(array('layout' => logistic_company_storage_get_array('sc_testimonials_data', 'style')));
		...
		$args = array(
			'layout' => logistic_company_storage_get_array('sc_testimonials_data', 'style'),
			'number' => logistic_company_storage_get_array('sc_testimonials_data', 'counter'),
			'columns_count' => logistic_company_storage_get_array('sc_testimonials_data', 'columns'),
			'slider' => logistic_company_storage_get_array('sc_testimonials_data', 'slider'),
			'show' => false,
			'descr'  => 0,
			'tag_id' => $id,
			'tag_class' => $class,
			'tag_animation' => '',
			'tag_css' => $css,
			'tag_css_wh' => logistic_company_storage_get_array('sc_testimonials_data', 'css_wh'),
			'author' => $author,
			'position' => $position,
			'link' => $link,
			'email' => $email,
		...
		return apply_filters('logistic_company_shortcode_output', $output, 'trx_testimonials_item', $atts, $content);
	}
	logistic_company_require_shortcode('trx_testimonials_item', 'logistic_company_sc_testimonials_item');
}
// ---------------------------------- [/trx_testimonials] ---------------------------------------
...
if (!function_exists('logistic_company_testimonials_reg_shortcodes')) {
	//Handler of add_filter('logistic_company_action_shortcodes_list',	'logistic_company_testimonials_reg_shortcodes');
	function logistic_company_testimonials_reg_shortcodes() {
		if (logistic_company_storage_isset('shortcodes')) {

			$testimonials_groups = logistic_company_get_list_terms(false, 'testimonial_group');
			$testimonials_styles = logistic_company_get_list_templates('testimonials');
			$controls = logistic_company_get_list_slider_controls();

			logistic_company_sc_map_before('trx_title', array(
			
				// Testimonials
				"trx_testimonials" => array(
					"title" => esc_html__("Testimonials", 'logistic-company'),
					"desc" => wp_kses_data( __("Insert testimonials into post (page)", 'logistic-company') ),
					"decorate" => true,
					"container" => false,
					"params" => array(
						"title" => array(
						...
						"style" => array(
							"title" => esc_html__("Testimonials style", 'logistic-company'),
							"desc" => wp_kses_data( __("Select style to display testimonials", 'logistic-company') ),
						...
						"columns" => array(
							...
							"desc" => wp_kses_data( __("How many columns use to show testimonials", 'logistic-company') ),
							"value" => 1,
							"min" => 1,
							"max" => 6,
							"step" => 1,
						...
						"slider" => array(
							...
							"desc" => wp_kses_data( __("Use slider to show testimonials", 'logistic-company') ),
							"value" => "yes",
							"type" => "switch",
						...
						"align" => array(
							...
							"desc" => wp_kses_data( __("Alignment of the testimonials block", 'logistic-company') ),
							"divider" => true,
							"value" => "",
							"type" => "checklist",
							"dir" => "horizontal",
						...
						"custom" => array(
							...
							"desc" => wp_kses_data( __("Allow get testimonials from inner shortcodes (custom) or get it from specified group (cat)", 'logistic-company') ),
							"divider" => true,
							"value" => "no",
							"type" => "switch",
							"options" => logistic_company_get_sc_param('yes_no')
						...
						"cat" => array(
							...
							"desc" => wp_kses_data( __("Select categories (groups) to show testimonials. If empty - select testimonials from any category (group) or from IDs list", 'logistic-company') ),
							"dependency" => array(
								'custom' => array('no')
							),
							"divider" => true,
							...
							"style" => "list",
							"multiple" => true,
							"options" => logistic_company_array_merge(array(0 => esc_html__('- Select category -', 'logistic-company')), $testimonials_groups)
						),
						"count" => array(
					...
					"children" => array(
						...
						"desc" => wp_kses_data( __("Testimonials item (custom parameters)", 'logistic-company') ),
						"container" => true,
						"params" => array(
							"author" => array(
								"title" => esc_html__("Author", 'logistic-company'),
								"desc" => wp_kses_data( __("Name of the testimonmials author", 'logistic-company') ),
								"value" => "",
								"type" => "text"
							),
							"link" => array(
								"title" => esc_html__("Link", 'logistic-company'),
								"desc" => wp_kses_data( __("Link URL to the testimonmials author page", 'logistic-company') ),
								"value" => "",
								"type" => "text"
							),
							"email" => array(
								"title" => esc_html__("E-mail", 'logistic-company'),
								"desc" => wp_kses_data( __("E-mail of the testimonmials author (to get gravatar)", 'logistic-company') ),
								"value" => "",
								"type" => "text"
							),
							"photo" => array(
								"title" => esc_html__("Photo", 'logistic-company'),
								"desc" => wp_kses_data( __("Select or upload photo of testimonmials author or write URL of photo from other site", 'logistic-company') ),
								"value" => "",
								"type" => "media"
							),
							"_content_" => array(
								"title" => esc_html__("Testimonials text", 'logistic-company'),
								"desc" => wp_kses_data( __("Current testimonials text", 'logistic-company') ),
								"divider" => true,
								"rows" => 4,
								"value" => "",
								"type" => "textarea"
...
if (!function_exists('logistic_company_testimonials_reg_shortcodes_vc')) {
	//Handler of add_filter('logistic_company_action_shortcodes_list_vc',	'logistic_company_testimonials_reg_shortcodes_vc');
	function logistic_company_testimonials_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Testimonials", 'logistic-company'),
				"description" => wp_kses_data( __("Insert testimonials slider", 'logistic-company') ),
				...
				"params" => array(
					array(
						"param_name" => "style",
						"heading" => esc_html__("Testimonials style", 'logistic-company'),
						"description" => wp_kses_data( __("Select style to display testimonials", 'logistic-company') ),
						"class" => "",
						"admin_label" => true,
					...
					array(
						...
						"description" => wp_kses_data( __("Alignment of the testimonials block", 'logistic-company') ),
						"class" => "",
					...
					array(
						...
						"description" => wp_kses_data( __("Allow get testimonials from inner shortcodes (custom) or get it from specified group (cat)", 'logistic-company') ),
						"class" => "",
					...
					array(
						...
						"description" => wp_kses_data( __("Select categories (groups) to show testimonials. If empty - select testimonials from any category (group) or from IDs list", 'logistic-company') ),
						...
						"class" => "",
						"value" => array_flip(logistic_company_array_merge(array(0 => esc_html__('- Select category -', 'logistic-company')), $testimonials_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
						"heading" => esc_html__("Columns", 'logistic-company'),
						"description" => wp_kses_data( __("How many columns use to show testimonials", 'logistic-company') ),
						"group" => esc_html__('Query', 'logistic-company'),
						"admin_label" => true,
						"class" => "",
						"value" => "1",
		...
		vc_map( array(
				...
				"name" => esc_html__("Testimonial", 'logistic-company'),
				"description" => wp_kses_data( __("Single testimonials item", 'logistic-company') ),
				"show_settings_on_create" => true,
				...
				"params" => array(
					array(
						"param_name" => "author",
						"heading" => esc_html__("Author", 'logistic-company'),
						"description" => wp_kses_data( __("Name of the testimonmials author", 'logistic-company') ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"description" => wp_kses_data( __("Link URL to the testimonmials author page", 'logistic-company') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"description" => wp_kses_data( __("E-mail of the testimonmials author", 'logistic-company') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						"param_name" => "photo",
						"heading" => esc_html__("Photo", 'logistic-company'),
						"description" => wp_kses_data( __("Select or upload photo of testimonmials author or write URL of photo from other site", 'logistic-company') ),
						"class" => "",
						"value" => "",
						"type" => "attach_image"
					),
					...
					array(
						"param_name" => "content",
						"heading" => esc_html__("Testimonials text", 'logistic-company'),
						"description" => wp_kses_data( __("Current testimonials text", 'logistic-company') ),
						"class" => "",
						"value" => "",
						"type" => "textarea_html"
					),
		...
		class WPBakeryShortCode_Trx_Testimonials extends LOGISTIC_COMPANY_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Testimonials_Item extends LOGISTIC_COMPANY_VC_ShortCodeCollection {}
