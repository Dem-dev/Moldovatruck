...
if ( !function_exists( 'logistic_company_template_related_theme_setup' ) ) {
	add_action( 'logistic_company_action_before_init_theme', 'logistic_company_template_related_theme_setup', 1 );
	function logistic_company_template_related_theme_setup() {
		logistic_company_add_template(array(
			'layout' => 'related',
			'mode'   => 'blog',
			'need_columns' => true,
			'need_terms' => true,
			'title'  => esc_html__('Related posts /no columns/', 'logistic-company'),
		...
		logistic_company_add_template(array(
			...
			'need_columns' => true,
			'need_terms' => true,
			'title'  => esc_html__('Related posts /2 columns/', 'logistic-company'),
			'thumb_title'  => esc_html__('Medium square image (crop)', 'logistic-company'),
			'w'		 => 370,
			'h'		 => 370
		));
		logistic_company_add_template(array(
			...
			'need_columns' => true,
			'need_terms' => true,
			'title'  => esc_html__('Related posts /3 columns/', 'logistic-company'),
			'thumb_title'  => esc_html__('Medium square image (crop)', 'logistic-company'),
			'w'		 => 370,
			'h'		 => 370
		));
		logistic_company_add_template(array(
			...
			'need_columns' => true,
			'need_terms' => true,
			'title'  => esc_html__('Related posts /4 columns/', 'logistic-company'),
			'thumb_title'  => esc_html__('Medium square image (crop)', 'logistic-company'),
			'w'		 => 370,
			'h'		 => 370
		));
...
// Template output
if ( !function_exists( 'logistic_company_template_related_output' ) ) {
	function logistic_company_template_related_output($post_options, $post_data) {
		...
		?>
		<<?php logistic_company_show_layout($tag); ?> class="post_item post_item_<?php echo esc_attr($style); ?> post_item_<?php echo esc_attr($post_options['number']); ?>">
				...
				<?php if ($show_title) { ?>
					<div class="post_content_wrap">
						...
						if (!isset($post_options['links']) || $post_options['links']) { 
							...
							?><h5 class="post_title"><?php logistic_company_show_layout($post_data['post_title']); ?></h5><?php
						}
